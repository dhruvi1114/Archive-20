...
| Adding or removing a person after booking | **Not supported** (client decision, 2026-08-27). To change who is attending, cancel the booking and book again. Editing a booking mid-flight would mean top-up invoices and credit notes against a bill the payer has already seen. |
| What kind of event it is | **A master the association maintains**, at Masters ▸ Event Types (client decision, 2026-08-27). Not a fixed list in code: a trade body adds "Buyer-Seller Meet" the week it decides to run one, and a list that changes only by release becomes a free-text box. Optional on an event — the master may be empty on day one, and events created before it existed have no honest value to give it. A type in use cannot be deleted, only deactivated; the events already carrying it keep it, because the type is part of what that event *was*. |
| How late registration may close | **The day before the event starts, at the latest** (client decision, 2026-08-27). For an event running 1–10 Sep the last selectable deadline is 31 Aug; the 1st is not offered. A booking taken on the morning of the event arrives after the badges are printed and the caterer's count has gone in, which is the reason a deadline exists at all. Left blank the field means the absence of a deadline — bookings stay open until the event begins — which is a different thing from a deadline landing on the start day. |
| How late a price window may run | **Up to the day registration closes** — never past it. A tier decides the price at the moment somebody books, so a window opening after the last day anybody can book is a rate nobody can be charged. Where Registration Closes is blank it closes at the event start, and that is the bound. The event's own end date does not enter into it. |
