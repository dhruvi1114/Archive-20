...
_Last updated: 2026-08-18 — **M0–M3 APPROVED. M4 BUILT + SELF-TESTED, awaiting user approval.** Pre-cycle tar snapshot: `~/assoc-snapshots/BI-Platform-2026-08-12-1032-m0-start.tar.gz` (R-13 control since git is deferred)._
...
| M5 | Payments, receipts, refunds, reconciliation | PENDING | B | OQ-4 (adapter covers it) | — | — |
| M6 | Membership renewal | PENDING | B | OQ-6 | — | — |
| M7 | Events | PENDING | B | — | — | — |
| M8 | Communication & notifications | PENDING | B | OQ-5 (adapter covers it) | — | — |
| M9 | Directory & public website | PENDING | A | OQ-7 | — | — |
...
Blocking later cycles: ~~OQ-1 (M1)~~ *(carried into M1 by user decision — the proposed matrix in `rbac.md` §3 is seeded as the working default; permissions are seed data, so a change is an upsert, not a migration)* · OQ-2 + OQ-9 (M2) · OQ-3 + OQ-8 (M4) · OQ-6 (M6) · OQ-7 (M9) · OQ-13 (M10).
